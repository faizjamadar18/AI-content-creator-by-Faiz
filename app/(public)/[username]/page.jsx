...
export default function ProfilePage({ params }) {
  const { username } = React.use(params);   // new way to expract username from URL in react 19+ 
  ...
  const { data: postsData, isLoading: postsLoading } = useConvexQuery(
    api.public.getPubishedPostByUsername,
    {
      username
    }
  );
  ...
  const { data: isFollowing } = useConvexQuery(
    ...
    user ? { followingId: user._id } : "skip"
  );
  // → "skip" meaning do not run the query (avoid errors).



  // Follow mutation
  const toggleFollow = useConvexMutation(api.follows.toggleFollow);
  // togglrFollow(object) : {mutate,data,isLoading,error}
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-12">
          ...
          {/*  Posts, Followers , Total Like , Total Views */}
          <div className="flex justify-center gap-8 mb-12">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-white">
                {postsData.reduce((sum, post) => sum + post.viewCount, 0)}
              </div>
              <div className="text-sm text-slate-400">Total Views</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-white">
                {postsData.reduce((sum, post) => sum + post.likeCount, 0)}
              </div>
